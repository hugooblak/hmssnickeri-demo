<?php
/**
 * HMS Snickeri AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "HMS Snickeri AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through hm_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'hmssnickeri', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'hmssnickeri' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'hmssnickeri' ) ) );

		register_block_pattern_category( 'hmssnickeri-sections', array( 'label' => __( 'HMS Snickeri AB sections', 'hmssnickeri' ) ) );
		register_block_pattern_category( 'hmssnickeri-pages', array( 'label' => __( 'HMS Snickeri AB page layouts', 'hmssnickeri' ) ) );
	}
);
