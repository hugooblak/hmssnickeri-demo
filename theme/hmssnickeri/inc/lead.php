 ...
 * or photo. They call hm_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional HMS Snickeri AB.
 *
 ...
 * @return string
 */
function hm_sample_note() {
	return hm_lead( 'ar_riktig' ) ? '' : ' <span class="hm-sample">(platshållare)</span>';
}
 ...
 * @return string File name for hm_lead_img().
 */
function hm_lead_bild( $i ) {
	$bilder = (array) hm_lead( 'bilder', array() );
 ...
 * @return string
 */
function hm_ring_label() {
	$namn = hm_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, hm_lead( 'telefon' ) ) : 'Ring ' . hm_lead( 'telefon' );
}
