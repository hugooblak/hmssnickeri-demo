...
add_filter(
	...
	function ( $exporters ) {
		$exporters['hmssnickeri-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'hmssnickeri-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'hmssnickeri-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'hmssnickeri-leads',
						'group_label' => __( 'Quote requests', 'hmssnickeri-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['hmssnickeri-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'hmssnickeri-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
