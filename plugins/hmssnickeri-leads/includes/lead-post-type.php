...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'hmssnickeri-leads' ),
					'singular_name' => __( 'Lead', 'hmssnickeri-leads' ),
					'edit_item'     => __( 'Förfrågan', 'hmssnickeri-leads' ),
					'search_items'  => __( 'Search leads', 'hmssnickeri-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'hmssnickeri-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'hmssnickeri-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'hmssnickeri-leads' ),
			'npl_phone'    => __( 'Phone', 'hmssnickeri-leads' ),
			'npl_service'  => __( 'Needs', 'hmssnickeri-leads' ),
			'npl_timeline' => __( 'When', 'hmssnickeri-leads' ),
			'npl_zip'      => __( 'ZIP', 'hmssnickeri-leads' ),
			'npl_source'   => __( 'Source', 'hmssnickeri-leads' ),
			'npl_status'   => __( 'Status', 'hmssnickeri-leads' ),
			'date'         => __( 'Received', 'hmssnickeri-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'hmssnickeri-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'hmssnickeri-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'hmssnickeri-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'hmssnickeri-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'hmssnickeri-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'hmssnickeri-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'hmssnickeri-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'hmssnickeri-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'hmssnickeri-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'hmssnickeri-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'hmssnickeri-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'hmssnickeri-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
